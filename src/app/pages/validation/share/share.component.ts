...
interface Version {
  ...
  entities: {
    [entityName: string]: {
      languages: any[]; // Définissez le type correct pour vos langues
      selectedLanguages: any[]; // Langues sélectionnées pour cette entité
    };
  };
}
...
export class ShareComponent implements OnInit {
  ...
  valider(item: any): void {
    ...
    const LanguageNames = item.selectedItems.map(entity => entity.selectedLanguages);
    const formattedLanguages = LanguageNames.map(languages => languages.map(language => ({ id: language.id, itemName: language.itemName })));


    console.log("elojole", LanguageNames);
    console.log("aaaaa", formattedLanguages);
    ...
    const serializedData = JSON.stringify(sharedSD);
    console.log("seria", serializedData);
  ...
  loadPublishedVersions(): void {
    ...
    for (let i = 1; i <= 3; i++) {
      this.publishedVersions.push({
        ...
        selectedLanguages: [],
        selectedItems: [],
        entities: {}// Initialisation des entités sélectionnées à vide
      });
    }
  ...
  onLanguageSelect(language: string, entity: any, version: Version) {
    entity.selectedLanguages.push(language);
    version.selectedItems2.push(entity.selectedLanguages);
  }

  // Méthode pour supprimer une langue sélectionnée d'une entité donnée dans une version donnée
  onLanguageDeselect(language: string, entity: any, version: Version) {
    entity.selectedLanguages = entity.selectedLanguages.filter(selectedLanguage => selectedLanguage !== language);
    version.selectedItems2.push(entity.selectedLanguages);
  }
